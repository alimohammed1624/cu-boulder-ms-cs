...
fun main(): Unit = runBlocking {
    ...
    var time = benchmark.start(this)
    fakeEmailServer.stop()
    var rate = getEnvInt("REGISTRATION_COUNT", 5_000) / time.inWholeSeconds
    if(rate < 10) {
        throw AssertionError("Less than 10 ( $rate ) registrations per second")
    }
}
